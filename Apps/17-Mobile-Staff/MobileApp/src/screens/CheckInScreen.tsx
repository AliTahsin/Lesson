...
export const CheckInScreen = ({ navigation }: any) => {
  ...
  const handleCheckIn = async () => {
    ...
    setLoading(true);
    try {
      const result = await staffApi.checkIn({
      ...
      Alert.alert(
        'Başarılı',
        `Check-in işlemi tamamlandı.\nDijital Anahtar: ${result.digitalKey}`,
        [{ text: 'Tamam', onPress: () => navigation.goBack() }]
      );
    } catch (error: any) {
      Alert.alert('Hata', error.response?.data?.error || 'Check-in işlemi başarısız');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.card}>
        <Text style={styles.title}>Check-in İşlemi</Text>
        ...
        >
          {loading ? (
            ...
            <Text style={styles.buttonText}>Check-in Yap</Text>
          )}
        </TouchableOpacity>
      </View>
    </ScrollView>
